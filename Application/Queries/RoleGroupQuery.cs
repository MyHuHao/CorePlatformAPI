    ...
    IRoleGroupMenuRepository repositoryMenuRepository)
{
    ...
    // 通过ID获取所有的资源关联
    public async Task<List<string>> GetAllRoleGroupResourceByIdAsync(string companyId, string roleGroupId)
    {
        var resources = await repositoryResRepository.GetAllRoleGroupByIdAsync(companyId, roleGroupId);
        return resources.Select(r => r.ResId).ToList();
    }

    // 通过ID获取所有的菜单关联
    public async Task<List<string>> GetAllRoleGroupWebMenuByIdAsync(string companyId, string roleGroupId)
    {
        var result = await repositoryMenuRepository.GetAllRoleGroupByIdAsync(companyId, roleGroupId);
        return result.Select(r => r.WebMenuId).ToList();
    }
